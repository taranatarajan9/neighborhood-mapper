...
// Storage for neighborhood colors (will be initialized from localStorage in a useEffect)
let neighborhoodColors = {};

// Function to initialize the colors from localStorage
export function initNeighborhoodColors() {
  if (typeof window !== 'undefined') {
    const storedColors = localStorage.getItem('neighborhoodColors');
    neighborhoodColors = storedColors ? JSON.parse(storedColors) : {};
  }
...
export function getNeighborhoodColor(name) {
  ...
  if (Object.keys(neighborhoodColors).length === 0 && typeof window !== 'undefined') {
