...
export async function getNeighborhoodColor(name) {
  const cacheKey = name.toLowerCase(); // Use lowercase only for cache key lookups
  
  // Check cache first
  if (neighborhoodColorsCache[cacheKey]) {
    return neighborhoodColorsCache[cacheKey];
  }

  // Check database - search case insensitively but preserve original case
  const { data, error } = await supabase
    .from('neighborhood_colors')
    .select('color')
  ...
  if (!error && data) {
    neighborhoodColorsCache[cacheKey] = data.color;
    return data.color;
  }

  // Generate new color if not found
  const newColor = generateRandomColor();
  neighborhoodColorsCache[cacheKey] = newColor;

  // Save to database - use original case
  const { error: insertError } = await supabase
    .from('neighborhood_colors')
    .insert({ name: name, color: newColor });
...
export function getAverageColorSync(names) {
  ...
  const colors = names.map(name => {
    const cacheKey = name.toLowerCase(); // Use lowercase only for cache key lookups
    return neighborhoodColorsCache[cacheKey] || '#cccccc';
  });
...
export function groupLocationsById(locations) {
  ...
  locations.forEach(location => {
    ...
    if (!grouped[id]) {
      ...
      // Merge names - preserve duplicates for accurate percentage calculations
      if (Array.isArray(location.names)) {
        location.names.forEach(name => {
          if (name) {
            // Add all names, including duplicates
